...
1) generate   (npm run generate:dashboard)
   ...
   ├─ collectWatchlist       관심 기업 5곳(애플/MSFT/엔비디아/아마존/알파벳) ── FMP API
...
대시보드 상단·하단에는 "투자 조언이 아닙니다" 배너가 항상 표시되고, Claude 인사이트 프롬프트에는 특정 종목의 매수/매도를 추천하지 말라는 규칙이 명시되어 있습니다. 인사이트 카드 하단에도 코드에서 고정된 안내 문구를 항상 덧붙여, AI 출력 내용만으로는 통제되지 않는 부분을 보강합니다.

### 대시보드 화면 구성 (초보자를 위한 안내)

1. ⚠️ 투자 조언이 아니라는 안내 배너
2. 📌 오늘의 요약 — 오늘 지수가 전반적으로 상승/하락/혼조였는지 요약 + 읽는 순서 안내
3. 🇺🇸 미국 증시 / 🇰🇷 국내 증시 — 지수가 무엇을 의미하는지 설명 + 상승▲/하락▼/보합 해석 라벨
4. 💱 외국인·기관 동향 / 💵 10년물 국채금리 — 각 지표가 왜 중요한지에 대한 설명
5. 🏢 오늘의 관심 기업 — 애플·마이크로소프트·엔비디아·아마존·알파벳(구글) 5곳의 오늘 주가 변동 (매수/매도 추천 아님, 참고용)
6. 🤖 AI 인사이트 — 초보자 눈높이로 풀어 쓴 시장 설명 + 투자 조언이 아니라는 안내
7. ⚠️ 하단 안내 배너 (반복)
...
├── collectors/   # Alpha Vantage / FMP REST API 클라이언트 + 수집기 (미국 지수, 국채금리, 관심 기업)
